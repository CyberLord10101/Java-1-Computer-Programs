...
public class MazeLoader {
    ...
    public boolean findPath(Point p) {
        ...
        //Test boundaries for easy solution
        ...
        //Down
        if (foundSolution == false && grid[p.x + 1][p.y].getBackground() == OPEN_COLOR) {
            grid[p.x + 1][p.y].setBackground(PATH_COLOR);
            foundSolution = findPath(new Point(p.x + 1, p.y));
        }

        //Right 
        if (foundSolution == false && grid[p.x][p.y + 1].getBackground() == OPEN_COLOR) {
